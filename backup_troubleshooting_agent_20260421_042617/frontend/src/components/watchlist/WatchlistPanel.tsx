...
import { WATCHLIST_SEED_GROUPS } from './watchlistSeed';
import { WatchlistGroup, WatchlistPanelProps, WatchlistTickerRow } from './watchlistTypes';
import {
  COLLAPSED_COLUMNS,
  EXPANDED_COLUMNS,
  formatCompactVolume,
...
const collapsedGridTemplate = '132px 86px 86px 72px 116px 132px 116px 132px';
const expandedGridTemplate = '132px 82px 86px 82px 70px 70px 70px 70px 70px 70px 70px 116px 132px 116px 132px 68px 74px 94px 74px 92px 104px 92px 92px';
...
export default function WatchlistPanel({ onLaunchArtifact, quoteRows = [] }: WatchlistPanelProps) {
  const liveQuoteMap = useMemo(
    () => new Map(quoteRows.map((row) => [row.symbol.toUpperCase(), row])),
    [quoteRows]
  );

  const groups = useMemo<WatchlistGroup[]>(() => {
    return WATCHLIST_SEED_GROUPS.map((group) => {
      const tickers = group.tickers.map((ticker) => {
        const live = liveQuoteMap.get(ticker.symbol.toUpperCase());
        return live
          ? {
              ...ticker,
              latest: live.latest,
      ...
      const summary = tickers.length
        ? {
            ...group.summary,
            latest: tickers.reduce((sum, row) => sum + row.latest, 0) / tickers.length,
            pctChange: tickers.reduce((sum, row) => sum + row.pctChange, 0) / tickers.length,
            ivPctl: tickers.reduce((sum, row) => sum + row.ivPctl, 0) / tickers.length,
          }
        : group.summary;

      return { ...group, tickers, summary };
    });
  }, [liveQuoteMap]);

  const [expandedGroupKey, setExpandedGroupKey] = useState<WatchlistGroup['key'] | null>(groups[0]?.key ?? null);
        ...
        <div style={{ minHeight: 0, overflow: 'auto' }}>
          <div style={collapsedHeaderStyle}>
            {COLLAPSED_COLUMNS.map((column) => (
              <div key={column} style={column === 'Symbol' ? { textAlign: 'left' } : { textAlign: 'right' }}>
                {column}
              </div>
            ))}
          </div>
              ...
              <Fragment key={group.key}>
                <div
                  ...
                  onClick={() => {
                    ...
                    if (group.tickers[0]) setHighlightedSymbol(group.tickers[0].symbol);
                  }}
                  title={`Expand ${group.label}`}
                >
                  <div style={stringCellStyle()}>{isExpanded ? '▾ ' : '▸ '}{summary.symbol}</div>
                  ...
                  <div style={numberCellStyle(summary.upperEmCostExShorts)}>{formatPrice(summary.upperEmCostExShorts)}</div>
                  <div style={{ ...numberCellStyle(1), textAlign: 'right' }}>{summary.lowerEmFly}</div>
                  <div style={numberCellStyle(summary.lowerEmCostExShorts)}>{formatPrice(summary.lowerEmCostExShorts)}</div>
                </div>
                  ...
                  <>
                    <div style={expandedHeaderStyle}>
                      {EXPANDED_COLUMNS.map((column) => (
                        <div key={`${group.key}-${column}`} style={column === 'Symbol' ? { textAlign: 'left' } : { textAlign: 'right' }}>
                          {column}
                        </div>
                      ))}
                    </div>
                    ...
                    {group.tickers.map((ticker) => {
                      ...
                      return (
                        <div
                          ...
                          style={{ ...tickerRowBaseStyle, background: isHighlighted ? '#131313' : '#080808' }}
                        ...
                        >
                          ...
                          <div style={numberCellStyle(ticker.relStr14d)}>{formatPlainNumber(ticker.relStr14d)}</div>
                          ...
                          <div style={numberCellStyle(ticker.upperEmCostExShorts)}>{formatPrice(ticker.upperEmCostExShorts)}</div>
                          <div style={{ ...numberCellStyle(1), textAlign: 'right' }}>{ticker.lowerEmFly}</div>
                          <div style={numberCellStyle(ticker.lowerEmCostExShorts)}>{formatPrice(ticker.lowerEmCostExShorts)}</div>
                          ...
                          <div style={{ ...stringCellStyle(), textAlign: 'right', color: '#cbd5e1' }}>{ticker.ttmSqueeze}</div>
                          <div style={numberCellStyle(ticker.adr14d)}>{formatPlainNumber(ticker.adr14d, 2)}</div>
                          <div style={numberCellStyle(ticker.optionsVol)}>{formatCompactVolume(ticker.optionsVol)}</div>
                          <div style={numberCellStyle(ticker.totalVol1m)}>{formatCompactVolume(ticker.totalVol1m)}</div>
                          <div style={numberCellStyle(ticker.callVol)}>{formatCompactVolume(ticker.callVol)}</div>
                          <div style={numberCellStyle(ticker.putVol)}>{formatCompactVolume(ticker.putVol)}</div>
                        </div>
                      );
                    })}
                  </>
