...
if(Test-Path $BackendPython){ & $BackendPython -m py_compile (Get-ChildItem -Path $BackendRoot -Recurse -Filter *.py | ForEach-Object { $_.FullName }); if($LASTEXITCODE -ne 0){ throw "Backend syntax check failed with exit code $LASTEXITCODE" } }
