...
if (!(Test-Path $appPath)) { throw "Could not find $appPath. Run this from the repo root." }
if (!(Test-Path $cssPath)) { throw "Could not find $cssPath. Run this from the repo root." }

$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$backupDir = Join-Path $root "frontend_backup_$stamp"
New-Item -ItemType Directory -Path $backupDir | Out-Null
Copy-Item $appPath (Join-Path $backupDir 'App.tsx.bak')
Copy-Item $cssPath (Join-Path $backupDir 'index.css.bak')
...
export default function App() {
  ...
  const kpiCards = useMemo(
    () => [
      { label: 'NET LIQ', value: netLiq !== null ? `$${netLiq.toLocaleString()}` : '—' },
      { label: 'BP', value: bp !== null ? `$${bp.toLocaleString()}` : '—' },
      { label: '25X', value: netLiq !== null ? `$${(netLiq * 25).toLocaleString(undefined, { maximumFractionDigits: 2 })}` : '—' },
...
Write-Host "Patched App.tsx and index.css"
