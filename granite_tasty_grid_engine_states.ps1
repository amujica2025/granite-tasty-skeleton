# granite_tasty_grid_engine_states.ps1
...
type LayoutState = 'default' | 'positionsExpanded' | 'sidesExpanded';
...
const layouts: Record<LayoutState, LayoutMap> = {
  default: {
    watchlist: { colStart: 1, colEnd: 2, rowStart: 1, rowEnd: 7 },
    infoHub: { colStart: 2, colEnd: 6, rowStart: 1, rowEnd: 4 },
    positions: { colStart: 2, colEnd: 6, rowStart: 4, rowEnd: 7 },
    scanners: { colStart: 6, colEnd: 7, rowStart: 1, rowEnd: 7 },
  },
  positionsExpanded: {
    watchlist: { colStart: 1, colEnd: 2, rowStart: 1, rowEnd: 7 },
    infoHub: { colStart: 2, colEnd: 6, rowStart: 1, rowEnd: 2 },
    positions: { colStart: 2, colEnd: 6, rowStart: 2, rowEnd: 7 },
    scanners: { colStart: 6, colEnd: 7, rowStart: 1, rowEnd: 7 },
  },
  sidesExpanded: {
    watchlist: { colStart: 1, colEnd: 3, rowStart: 1, rowEnd: 7 },
    infoHub: { colStart: 3, colEnd: 5, rowStart: 1, rowEnd: 4 },
    positions: { colStart: 3, colEnd: 5, rowStart: 4, rowEnd: 7 },
    scanners: { colStart: 5, colEnd: 7, rowStart: 1, rowEnd: 7 },
  },
};
...
export default function App() {
  ...
  const collapsedWatchlist = layoutState !== 'sidesExpanded';
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <button
            ...
            style={{
              ...
              background: layoutState === 'default' ? '#222' : '#0b0b0b',
              color: '#ddd',
              padding: '6px 10px',
              cursor: 'pointer',
            }}
          ...
          <button
            onClick={() => setLayoutState('positionsExpanded')}
            style={{
              border: '1px solid #333',
              background: layoutState === 'positionsExpanded' ? '#222' : '#0b0b0b',
              color: '#ddd',
              padding: '6px 10px',
              cursor: 'pointer',
            }}
          ...
          <button
            onClick={() => setLayoutState('sidesExpanded')}
            style={{
              border: '1px solid #333',
              background: layoutState === 'sidesExpanded' ? '#222' : '#0b0b0b',
              color: '#ddd',
              padding: '6px 10px',
              cursor: 'pointer',
            }}
        ...
        <div style={{ ...panelStyle, ...areaStyle(layout.watchlist) }}>
          <div
            style={{
              height: 38,
        ...
        <div style={{ ...panelStyle, ...areaStyle(layout.infoHub) }}>
          <div
            style={{
              height: 30,
              borderBottom: '1px solid #242424',
          ...
          >
            ...
            >
              {kpiCards.concat(kpiCards).map((card, i) => (
                ...
                >
                  <div style={{ fontSize: 10, color: '#8f8f8f', textTransform: 'uppercase' }}>{card.label}</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: card.tone }}>{card.value}</div>
                  <div style={{ height: 16, border: '1px solid #1e1e1e', background: '#080808' }} />
                </div>
              ))}
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10 }}>
                    ...
                    <span style={{ color: card.chgPct >= 0 ? '#22c55e' : '#ef4444' }}>
                      {card.chgPct >= 0 ? '+' : ''}{card.chgPct.toFixed(2)}%
                    </span>
                  </div>
                  <div style={{ fontSize: 18, fontWeight: 700 }}>{card.last.toFixed(2)}</div>
                  <div style={{ height: 16, border: '1px solid #1e1e1e', background: '#080808', overflow: 'hidden' }}>
                    <svg viewBox="0 0 120 16" preserveAspectRatio="none" style={{ width: '100%', height: '100%' }}>
                      <polyline fill="none" stroke="#22c55e" strokeWidth="1.5" points="0,12 10,10 20,11 30,8 40,9 50,7 60,6 70,7 80,4 90,5 100,3 110,4 120,2" />
                    </svg>
                  </div>
                </div>
              ))}
        ...
        <div style={{ ...panelStyle, ...areaStyle(layout.positions) }}>
          <PositionsPanel netLiq={netLiq} />
        </div>

        <div style={{ ...panelStyle, ...areaStyle(layout.scanners) }}>
          <div
            style={{
              height: '100%',
              display: 'flex',
...
Write-Host ""
Write-Host "Applied grid engine states to App.tsx" -ForegroundColor Green
Write-Host ""
