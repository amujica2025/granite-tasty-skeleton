...
import type { WatchlistGroup, WatchlistGroupKey, WatchlistParentSummaryRow, WatchlistTickerRow } from './watchlistTypes';
...
export function buildParentSummaryRow(
  ...
  const avg = (selector: (row: WatchlistTickerRow) => number): number => {
    const nums = tickers.map(selector).filter((value) => Number.isFinite(value));
  ...
  return {
    symbol: label,
    latest: avg((row) => row.latest),
    pctChange: avg((row) => row.pctChange),
    ivPctl: avg((row) => row.ivPctl),
    upperEmFly: representative.upperEmFly,
    upperEmCostExShorts: avg((row) => row.upperEmCostExShorts),
    lowerEmFly: representative.lowerEmFly,
    lowerEmCostExShorts: avg((row) => row.lowerEmCostExShorts),
  };
}
