...
import type { Artifact } from './components/artifact/artifactTypes';
...
export default function App() {
  ...
  const artifactStore = useArtifactStore() as any;

  const ScannerPanelMount = ScannerPanel as React.ComponentType<any>;
  const JournalPopupMount = JournalPopup as React.ComponentType<any>;
  const ArtifactModalMount = ArtifactModal as React.ComponentType<any>;

  const activeArtifact = (artifactStore?.activeArtifact ??
    artifactStore?.currentArtifact ??
    artifactStore?.artifact ??
    null) as Artifact | null;

  const artifactIsOpen = Boolean(
    artifactStore?.isOpen ??
      artifactStore?.isModalOpen ??
      artifactStore?.modalOpen ??
      artifactStore?.openArtifactId ??
      activeArtifact
  );
  ...
  const closeArtifact = () => {
    ...
    if (typeof artifactStore?.closeModal === 'function') {
      artifactStore.closeModal();
      return;
    }
    if (typeof artifactStore?.setIsOpen === 'function') {
      artifactStore.setIsOpen(false);
      return;
    }
    if (typeof artifactStore?.setModalOpen === 'function') {
      artifactStore.setModalOpen(false);
    }
  };

  const openArtifactTestHook = () => {
    const testArtifact: Artifact = {
      id: 'artifact_test_hook',
      title: 'Artifact Test Hook',
      type: 'chart',
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    } as Artifact;

    if (typeof artifactStore?.open === 'function') {
      artifactStore.open(testArtifact);
      return;
    }
    if (typeof artifactStore?.openModal === 'function') {
      artifactStore.openModal(testArtifact);
      return;
    }
    if (typeof artifactStore?.setArtifact === 'function') {
      artifactStore.setArtifact(testArtifact);
    }
    if (typeof artifactStore?.setCurrentArtifact === 'function') {
      artifactStore.setCurrentArtifact(testArtifact);
    }
    if (typeof artifactStore?.setIsOpen === 'function') {
      artifactStore.setIsOpen(true);
      return;
    }
    if (typeof artifactStore?.setModalOpen === 'function') {
      artifactStore.setModalOpen(true);
    }
  };
        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                style={{
                  ...
                  minHeight: 0,
                  display: 'grid',
                  gridTemplateRows: '132px minmax(0, 1fr) 112px',
                  gap: 8,
                  padding: 8,
                  color: '#111',
                }}
              >
                ...
                >
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.04em' }}>QUICK RULE BUILDER</div>
                  ...
                  >
                    <div>
                      <div style={{ fontSize: 10, marginBottom: 4, fontWeight: 700 }}>SYMBOL</div>
                      <input
                        value={builderSymbol}
                        onChange={(e) => setBuilderSymbol(normalizeSymbol(e.target.value))}
                        style={inputBaseStyle}
                      />
                    </div>

                    <div>
                      <div style={{ fontSize: 10, marginBottom: 4, fontWeight: 700 }}>FIELD</div>
                      <select
                        value={builderField}
                      ...
                      >
                        ...
                        <option value="iv_pctl">IV Percentile</option>
                        <option value="imp_vol">Implied Vol</option>
                      </select>
                    </div>

                    <div>
                      <div style={{ fontSize: 10, marginBottom: 4, fontWeight: 700 }}>OPERATOR</div>
                      <select
                        value={builderOperator}
                        onChange={(e) => setBuilderOperator(e.target.value as AlertRuleOperator)}
                        style={inputBaseStyle}
                      >
                        <option value="<">&lt;</option>
                        <option value="<=">&lt;=</option>
                    ...
                    <div>
                      <div style={{ fontSize: 10, marginBottom: 4, fontWeight: 700 }}>VALUE</div>
                      <input
                        value={builderTargetValue}
                        onChange={(e) => setBuilderTargetValue(e.target.value)}
                        style={inputBaseStyle}
                      />
                    </div>
                ...
                >
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '1.4fr 0.9fr 0.8fr 0.8fr auto',
                      alignItems: 'center',
                      padding: '0 8px',
                  ...
                  <div style={{ minHeight: 0, overflow: 'auto' }}>
                    {alertRules.length === 0 ? (
                      <div style={{ padding: 10, fontSize: 11, color: '#231f20' }}>No active rules yet.</div>
                    ) : (
                      alertRules.map((rule) => (
                        <div
                          key={rule.alert_id}
                        ...
                        >
                          <div>
                            <div style={{ fontWeight: 700 }}>{rule.symbol}</div>
                            <div style={{ fontSize: 10, opacity: 0.8 }}>{getOperatorLabel(rule.operator)}</div>
                          </div>
                          <div>{getFieldLabel(rule.field_name)}</div>
                          <div>{rule.target_value}</div>
                          <div
                            style={{
                              fontWeight: 700,
                              color: rule.is_enabled ? '#14532d' : '#7f1d1d',
                            }}
                          >
                          ...
                          <div style={{ display: 'flex', gap: 4 }}>
                            <button
                              onClick={() => toggleAlertRuleEnabled(rule.alert_id)}
                              style={tinyActionButtonStyle}
                            >
                              {rule.is_enabled ? 'Disable' : 'Enable'}
                            </button>
                            <button onClick={() => deleteAlertRule(rule.alert_id)} style={tinyActionButtonStyle}>
                              Remove
                            </button>
                          </div>
                        </div>
                ...
                >
                  ...
                  >
                    ...
                    <span>{alertHistory.length} events</span>
                  </div>

                  <div style={{ minHeight: 0, overflow: 'auto', padding: 6 }}>
                    {renderedHistory.length === 0 ? (
                      <div style={{ fontSize: 11, color: '#231f20' }}>Nothing triggered yet.</div>
                    ) : (
                      renderedHistory.map((event) => (
                        <div
                          ...
                          style={{
                            display: 'grid',
                            gridTemplateColumns: '0.9fr 0.8fr 1fr auto',
                            gap: 6,
                            alignItems: 'center',
                            fontSize: 10,
                            padding: '4px 2px',
                        ...
                        >
                          <div style={{ fontWeight: 700 }}>{event.symbol}</div>
                          <div>{event.event_type.toUpperCase()}</div>
                          <div>
                          ...
                          {event.event_type === 'triggered' ? (
                            <button
                              onClick={() => dismissAlertEvent(event.alert_event_id)}
                              style={tinyActionButtonStyle}
                            >
                              Dismiss
                            </button>
            ...
            >
              ...
              >
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 6 }}>
                  <input
                    ...
                    placeholder="Add watched symbol"
                    style={{
                      ...inputBaseStyle,
                      color: '#0c0c0c',
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 0.8fr auto', gap: 6 }}>
                  <input
                    value={newStructureLabel}
                    onChange={(e) => setNewStructureLabel(e.target.value)}
                    placeholder="Add watched structure"
                    style={{
                      ...inputBaseStyle,
                      color: '#0c0c0c',
                      background: 'rgba(255,255,255,0.55)',
                  ...
                  />
                  <input
                    value={newStructureUnderlying}
                    onChange={(e) => setNewStructureUnderlying(e.target.value.toUpperCase())}
              ...
              {activeStructureWatches.length > 0 && (
                <div
                  style={{
                    padding: '6px 10px',
                    borderBottom: '1px solid rgba(0,0,0,0.18)',
                ...
                >
                  <div style={{ fontSize: 10, fontWeight: 700, marginBottom: 4, letterSpacing: '0.04em' }}>
                    WATCHED STRUCTURES
                  </div>

                  <div style={{ display: 'grid', gap: 4 }}>
                    {activeStructureWatches.map((watch) => {
                      const isPinned = pinnedInfoHubKeys.includes(`structure:${watch.watch_id}`);
                      return (
                        <div
                          key={watch.watch_id}
                          style={{
                        ...
                        >
                          <div>
                            <div style={{ fontWeight: 700 }}>{watch.label}</div>
                            <div>{watch.underlying}</div>
                          </div>

                          <div style={{ display: 'flex', gap: 4 }}>
                            <button
                              onClick={() => toggleStructureInfoHubPromotion(watch.watch_id)}
                              style={tinyActionButtonStyle}
                            >
                              {isPinned ? 'Unpin' : 'Pin'}
                            </button>
                            <button
                              onClick={() => upsertWatchObject({ ...watch, is_active: false })}
                              style={tinyActionButtonStyle}
                            >
                              Hide
                            </button>
              ...
              <div style={{ flex: 1, overflow: 'auto', padding: '8px 10px' }}>
                {!watchlistExpanded ? (
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '1fr 1fr 1fr 0.85fr auto',
                      fontSize: 11,
                      gap: 0,
                      color: '#0c0c0c',
                    }}
                  >
                    {['SYMBOL', 'LAST', '%CHG', 'WATCH', 'ACTIONS'].map((label) => (
                      <div
                        key={label}
                        style={{
                          fontWeight: 700,
                    ...
                    {watchlistRows.flatMap((row) => {
                      const watched = isSymbolWatched(row.symbol);
                      const pinned = pinnedInfoHubKeys.includes(`symbol:${row.symbol}`);

                      return [
                        <div
                          key={`${row.symbol}-s`}
                          style={{ padding: '4px 2px', borderBottom: '1px solid #101010' }}
                        >
                          {row.symbol}
                        </div>,
                        <div
                        ...
                        <div
                          ...
                          style={{
                            padding: '4px 2px',
                            color: row.pctChange >= 0 ? '#166534' : '#991b1b',
                            borderBottom: '1px solid #101010',
                          }}
                        >
                        ...
                        <div
                          key={`${row.symbol}-w`}
                          style={{ padding: '4px 2px', borderBottom: '1px solid #101010', fontWeight: 700 }}
                        >
                          {watched ? 'ON' : 'OFF'}
                        </div>,
                        <div
                        ...
                        >
                          <button onClick={() => addQuickAlertForSymbol(row.symbol)} style={tinyActionButtonStyle}>
                            Alert
                          </button>
                          <button onClick={() => toggleSymbolWatch(row.symbol)} style={tinyActionButtonStyle}>
                            {watched ? 'Unwatch' : 'Watch'}
                          </button>
                          <button onClick={() => promoteSymbolToInfoHub(row.symbol)} style={tinyActionButtonStyle}>
                            {pinned ? 'Unpin' : 'Pin'}
                          </button>
                        </div>,
                      ];
                  ...
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: 'repeat(19, minmax(86px, 1fr)) 168px',
                      fontSize: 10,
                      gap: 0,
                      color: '#0c0c0c',
                    }}
                    ...
                    {watchlistRows.map((row) => {
                      ...
                      return (
                        <Fragment key={row.symbol}>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>{row.symbol}</div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.latest.toFixed(2)}
                          </div>
                          <div
                            style={{
                          ...
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.relStr14d.toFixed(1)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.ivPctl.toFixed(0)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.ivHv.toFixed(2)}
                          </div>
                          ...
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.bbRank.toFixed(0)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.ttmSqueeze}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {row.adr14d.toFixed(1)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {formatCompact(row.optionsVol)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {formatCompact(row.totalVol1m)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {formatCompact(row.callVol)}
                          </div>
                          <div style={{ padding: '4px 6px', borderBottom: '1px solid #101010' }}>
                            {formatCompact(row.putVol)}
                          </div>
                          <div
                            style={{
                              padding: '2px 4px',
            ...
            >
              ...
              >
                ...
                <div>Artifact modal: {artifactIsOpen ? 'Open' : 'Closed'}</div>
              </div>
            </div>
          </div>
        </div>
