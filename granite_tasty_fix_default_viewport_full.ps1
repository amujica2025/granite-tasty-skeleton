              ...
              {!watchlistExpanded ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
                  ...
                  {watchlistRows.flatMap((row) => [
                    ...
                    <div key={`${row.symbol}-p`} style={{ padding: '6px 5px', fontSize: 10, color: row.pctChange >= 0 ? '#22c55e' : '#ef4444', borderBottom: '1px solid #101010' }}>{row.pctChange >= 0 ? '+' : ''}{row.pctChange.toFixed(2)}%</div>,
